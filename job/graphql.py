...
def export_jsonl(context, language=None):
  """
  ...
  # Choose which query file to use based on language parameter
  query_file = 'export_data_job_translations.graphql' if language else 'export_data_job.graphql'
  query = Path(f'./graphql_queries/{query_file}').read_text()

  # If language is provided, replace the template variable
  if language:
...
def get_shopify_jsonl_fp(shop_url, api_version, token, output_dir, run_num="", multiMarket=False, shopify_market=None, shopify_language=None):
  """
  ...
  context = {}
  polling.poll(
    lambda: export_jsonl(context, language=shopify_language if multiMarket else None),
    step=20,
    timeout=7200
  )
