...
2. Transforms the data through several stages to match Bloomreach's requirements
3. Uploads the transformed data to your Bloomreach Discovery catalog

## Prerequisites

- A Shopify store with Admin API access
- A Shopify Personal Access Token (PAT) with the necessary permissions
...
- **Authentication Errors**: Verify your Shopify PAT and Bloomreach API token are correct and have sufficient permissions
- **Processing Failures**: Check the container logs for detailed error messages
- **Feed API Errors**: Ensure your Bloomreach account ID and catalog name are correct

## Limitations

- The integration processes all products in a single batch, which may cause timeouts for very large catalogs
- Product variants must have unique SKUs or IDs for proper mapping
- Custom fields are prefixed to avoid collisions with Bloomreach reserved attributes

## Build Custom Image

To build a custom version of this container:
